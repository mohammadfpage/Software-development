...
git --version
```
# تنظیم نام و ایمیل برای اولین Commit
```bash
...
git init
```
# دریافت آخرین تغییرات از مخزن اصلی
```bash
git pull
```
# بررسی وضعیت فایل‌ها
...
git diff
```
# بازگشت به یک Commit خاص (شناسه 7 رقم اول)
```bash
git checkout 1a2b3c4
```
## یا بازگشت کامل پروژه به حالت آن commit
bash git reset git reset --soft <commit-hash> git reset --hard <commit-hash> git checkout <commit-hash>
